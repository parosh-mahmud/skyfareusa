...
export default function ModifySearchForm() {
  const router = useRouter();
  const searchParams = useSearchParams();

  // This helper function reads the state from the URL's query parameters.
  // It provides safe defaults if a parameter is missing or invalid.
  const initializeStateFromURL = () => {
    try {
      return {
        tripType: searchParams.get("tripType") || "one-way",
        cabinClass: searchParams.get("cabinClass") || "economy",
        slices: JSON.parse(searchParams.get("slices")) || [],
        passengers: JSON.parse(searchParams.get("passengers")) || [],
      };
    } catch (e) {
      console.error("Could not parse URL parameters:", e);
      // Return safe, empty defaults to prevent crashing the component.
      return {
        tripType: "one-way",
        cabinClass: "economy",
        slices: [],
  ...
  // The component's state is now initialized directly from the URL.
  const [tripType, setTripType] = useState(
    () => initializeStateFromURL().tripType
  );
  const [slices, setSlices] = useState(() => initializeStateFromURL().slices);
  const [passengers, setPassengers] = useState(
    () => initializeStateFromURL().passengers
  );
  const [cabinClass, setCabinClass] = useState(
    () => initializeStateFromURL().cabinClass
  );
  ...
  // This effect keeps the form in sync with the URL if the user navigates using browser buttons.
  useEffect(() => {
    const stateFromURL = initializeStateFromURL();
    setTripType(stateFromURL.tripType);
    setSlices(stateFromURL.slices);
    setPassengers(stateFromURL.passengers);
    setCabinClass(stateFromURL.cabinClass);
  }, [searchParams]);

  // --- Airport Lookup (No changes needed) ---
  function fetchAirports(q) {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    if (q.length < 2) return setSuggestions([]);
    setLoading(true);
  ...
  // --- Submit handler now updates the URL ---
  function handleSubmit(e) {
    e.preventDefault();
  ...
  // --- Close dropdown on outside click (No changes needed) ---
  useEffect(() => {
    const onClickOutside = (e) => {
      if (
        dropdownRef.current &&
  ...
  // Safety check: Prevent rendering errors if slices aren't loaded yet.
  if (slices.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 h-[150px] flex items-center justify-center">
        <p className="text-gray-500">Loading search options...</p>
      </div>
    );
  }
