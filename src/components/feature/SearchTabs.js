        ...
        <TabsList className="h-auto bg-background p-1 rounded-2xl shadow-lg">
          {services.map((service) => (
            <TabsTrigger
              key={service.id}
              value={service.id}
              // UPDATED: Padding is adjusted for icon-only and full-text views
              className="p-3 sm:px-4 sm:py-3 data-[state=active]:bg-transparent data-[state=active]:shadow-none font-medium text-muted-foreground data-[state=active]:text-foreground data-[state=active]:font-semibold relative"
            >
              {/* UPDATED: Icon margin is now responsive */}
              <service.icon size={18} className="sm:mr-2" />
              {/* UPDATED: Text label is hidden on small screens (less than 640px) */}
