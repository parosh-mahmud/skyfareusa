...
export default function TravellerDetailsStep({ onNext, passengers = [] }) {
  ...
  const validateForm = () => {
    ...
    travellerData.forEach((traveller, index) => {
      if (!traveller.firstName) newErrors[`${index}-firstName`] = "First name is required"
      if (!traveller.lastName) newErrors[`${index}-lastName`] = "Last name is required"
      if (!traveller.dateOfBirth) newErrors[`${index}-dateOfBirth`] = "Date of birth is required"
      if (!traveller.gender) newErrors[`${index}-gender`] = "Gender is required"
      if (index === 0) {
  ...
  const handleInputChange = (index, field, value) => {
    setTravellerData((prev) => prev.map((traveller, i) => (i === index ? { ...traveller, [field]: value } : traveller)))
    ...
    <div className="space-y-6">
      ...
      {travellerData.map((traveller, index) => (
        <div key={index} className="border rounded-lg p-4 space-y-4">
          <h3 className="font-medium">{index === 0 ? "Primary Contact" : `Passenger ${index + 1}`}</h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor={`title-${index}`}>Title</Label>
              <Select onValueChange={(value) => handleInputChange(index, "title", value)}>
                <SelectTrigger>
            ...
            <div>
              ...
              {errors[`${index}-firstName`] && (
                <p className="text-red-500 text-sm mt-1">{errors[`${index}-firstName`]}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors[`${index}-lastName`] && (
                <p className="text-red-500 text-sm mt-1">{errors[`${index}-lastName`]}</p>
              )}
            </div>
            ...
            <div>
              ...
              <Input
                ...
                className={errors[`${index}-dateOfBirth`] ? "border-red-500" : ""}
              />
              {errors[`${index}-dateOfBirth`] && (
                <p className="text-red-500 text-sm mt-1">{errors[`${index}-dateOfBirth`]}</p>
              )}
            </div>

            <div>
              <Label htmlFor={`gender-${index}`}>Gender *</Label>
              <Select onValueChange={(value) => handleInputChange(index, "gender", value)}>
                <SelectTrigger className={errors[`${index}-gender`] ? "border-red-500" : ""}>
