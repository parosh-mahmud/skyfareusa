...
import { Duffel } from "@duffel/api";
// import Amadeus from "amadeus"; // Amadeus client initialization commented out
import { NextResponse } from "next/server";

// Initialize the API clients
const duffel = new Duffel({ token: process.env.DUFFEL_ACCESS_TOKEN });
// const amadeus = new Amadeus({ // Amadeus client initialization commented out
//   clientId: process.env.AMADEUS_API_KEY,
//   clientSecret: process.env.AMADEUS_API_SECRET,
// });
...
export async function POST(request) {
  try {
    const { offerId, offer, sourceApi } = await request.json(); // Validate the incoming request to ensure it has the necessary data
    ...
    let pricedOffer; // Use a switch to handle the different logic for each API provider

    switch (sourceApi) {
      case "duffel":
        console.log(`✈️  Repricing Duffel offer ID: ${offerId}`); // For Duffel, we get the latest version of the offer using its ID.
        const duffelResponse = await duffel.offers.get(offerId); // The response is already in a good format. We just add our sourceApi key.

        pricedOffer = { ...duffelResponse.data, sourceApi: "duffel" };
        break; /* Amadeus case is now commented out
      case "amadeus":
        console.log(`✈️  Repricing Amadeus offer ID: ${offer.id}`);
...
        break;
      */
      ...
      default:
        // If the sourceApi is "amadeus", it will now fall through to this default case
        if (sourceApi === "amadeus") {
          return NextResponse.json(
            { success: false, error: "Amadeus API is currently disabled." },
            { status: 400 }
          );
        }
        return NextResponse.json(
