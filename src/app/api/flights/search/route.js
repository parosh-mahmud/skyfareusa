...
import { Duffel } from "@duffel/api";
// import Amadeus from "amadeus"; // Commented out
import { NextResponse } from "next/server";
...
const duffel = new Duffel({ token: process.env.DUFFEL_ACCESS_TOKEN });
// const amadeus = new Amadeus({ // Commented out
//   clientId: process.env.AMADEUS_API_KEY,
//   clientSecret: process.env.AMADEUS_API_SECRET,
//   hostname: "production",
// });
 ...
 * @returns {object} The payload for Amadeus's flightOffersSearch.post.
 */
// function createAmadeusPayload(req) { // Commented out
//   const mapPassengerType = (type) => {
//     const typeMap = {
//       adult: "ADULT",
//       child: "CHILD",
 ...
 * @returns {object} A normalized flight offer object.
 */
// const normalizeAmadeusOffer = (offer, dictionaries) => { // Commented out
//   return {
//     id: offer.id,
//     sourceApi: "amadeus",
//     total_amount: offer.price.total,
...
export async function POST(request) {
  try {
    ...
    const duffelPayload = createDuffelPayload(unifiedRequest);
    // const amadeusPayload = createAmadeusPayload(unifiedRequest); // Commented out

    const [duffelResult] = await Promise.allSettled([
      duffel.offerRequests.create({
        ...duffelPayload,
        max_connections: 1,
        return_offers: true,
      }),
      // amadeus.shopping.flightOffersSearch.post(amadeusPayload), // Commented out
    ]);
    ...
    // Process Amadeus results
    // if (amadeusResult.status === "fulfilled" && amadeusResult.value.result) { // Commented out
    //   const { data, dictionaries } = amadeusResult.value.result;
    //   if (data) {
    //     allOffers.push(
    //       ...data.map((offer) => normalizeAmadeusOffer(offer, dictionaries))
