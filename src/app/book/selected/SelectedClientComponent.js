    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select
            value={passengerData.gender || ""}
            onChange={(e) =>
              setPassengerData({ ...passengerData, gender: e.target.value })
            }
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
      ...
      <div className="mt-8">
        <button
          ...
          className="w-full bg-blue-600 text-white py-4 rounded-lg font-semibold hover:bg-blue-700 transition-colors"
        >
          Continue to Baggage & Seat Selection
        </button>
      </div>
    </div>
  );
...
const ExtraBaggagesStep = ({ onNext, offer }) => {
  const { setSelectedServices } = useBookingStore();
  const [isLoading, setIsLoading] = useState(false);

  console.log(
    "[v0] ExtraBaggagesStep - Offer:",
    offer?.id || "No offer available"
  );
  ...
  const AmadeusBaggageSeatSelection = () => {
    const [selectedBaggage, setSelectedBaggage] = useState({});
    const [selectedSeats, setSelectedSeats] = useState({});

    const baggageOptions = [
      { id: "none", name: "No Extra Baggage", price: 0 },
      { id: "20kg", name: "20kg Extra Baggage", price: 45 },
      { id: "30kg", name: "30kg Extra Baggage", price: 75 },
    ];

    const seatOptions = [
      { id: "none", name: "Random Seat Assignment", price: 0 },
      { id: "standard", name: "Standard Seat Selection", price: 15 },
      { id: "premium", name: "Premium Seat with Extra Legroom", price: 35 },
      { id: "exit", name: "Exit Row Seat", price: 25 },
    ];

    const handleConfirm = () => {
      const services = [];
      if (selectedBaggage.id !== "none") {
        services.push({
          type: "baggage",
    ...
    return (
      <div className="space-y-6">
            ...
            {baggageOptions.map((option) => (
              ...
              >
                <input
                  ...
                  name="baggage"
                  value={option.id}
                  checked={selectedBaggage.id === option.id}
                  onChange={() => setSelectedBaggage(option)}
                  className="w-4 h-4 text-blue-600"
                />
            ...
            {seatOptions.map((option) => (
              ...
              >
                <input
                  ...
                  name="seat"
                  value={option.id}
                  checked={selectedSeats.id === option.id}
                  onChange={() => setSelectedSeats(option)}
                  className="w-4 h-4 text-blue-600"
                />
                <div className="ml-3 flex-1">
                  <div className="flex justify-between">
        ...
        <div className="flex gap-3 pt-4">
          ...
          >
            {isLoading ? "Loading Services..." : "Continue to Next Step"}
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
        ...
        <div className="text-center py-8">
          <p className="text-gray-600 mb-4">
            No flight offer available. Please go back and select a flight.
          </p>
          <button
...
const ReviewPaymentStep = ({
  ...
  const handleBookNowPayLater = async () => {
    if (!agreedToTerms) return;
    ...
    try {
      ...
      const bookingData = {
        ...
        services: selectedServices,
        totalAmount: calculateTotalPrice(),
        paymentOption: "book-now-pay-later",
        status: "confirmed",
  ...
  const calculateTotalPrice = () => {
    ...
    if (offer?.total_amount) {
      basePrice = Number.parseFloat(offer.total_amount);
    } else if (offer?.rawOffer?.price?.total) {
      basePrice = Number.parseFloat(offer.rawOffer.price.total);
    }
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          disabled={!agreedToTerms || isProcessing}
          className="w-full bg-orange-100 text-orange-600 py-4 rounded-lg font-semibold hover:bg-orange-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {isProcessing ? (
            <div className="flex items-center justify-center">
        ...
        <div className="text-center text-sm text-gray-500 flex items-center justify-center gap-2">
          <svg
            ...
            viewBox="0 0 24 24"
          >
            <path d="M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.992 3.757 7.218c0 4.039 2.467 5.76 6.476 7.219 2.585.92 3.445 1.574 3.445 2.583 0 .98-.84 1.545-2.354 1.545-1.875 0-4.965-.921-6.99-2.109l-.9 5.555C5.175 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
          </svg>
...
const SelectedClientComponent = () => {
  const timeRemaining = useTimeRemaining(25);
  const [currentStep, setCurrentStep] = useState(1);
  const [passengerData, setPassengerData] = useState({});
  const { pricedOffer, selectedServices, setPricedOffer } = useBookingStore();
  const { selectedOffer, setSelectedOffer } = usePricingStore();
  const searchParams = useSearchParams();
  const router = useRouter();

  useEffect(() => {
    const offerData = searchParams.get("offer");
    if (offerData && !pricedOffer && !selectedOffer) {
      try {
        const parsedOffer = JSON.parse(decodeURIComponent(offerData));
        console.log("[v0] Setting offer from URL:", parsedOffer);
        setPricedOffer(parsedOffer);
        setSelectedOffer(parsedOffer);
      } catch (error) {
        console.error("[v0] Error parsing offer data:", error);
      }
    }
  }, [
    searchParams,
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <ExtraBaggagesStep onNext={handleNextStep} offer={displayOffer} />
        );
      case 3:
        return (
          <ReviewPaymentStep
...
export default SelectedClientComponent;
